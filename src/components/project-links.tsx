...
const FileTextIcon: IconComponent = ({ className }) => (
  <svg
    aria-hidden="true"
    viewBox="0 0 24 24"
    className={className}
...
const BadgeCheckIcon: IconComponent = ({ className }) => (
  <svg
    aria-hidden="true"
    viewBox="0 0 24 24"
    className={className}
...
const ExternalLinkIcon: IconComponent = ({ className }) => (
  <svg
    aria-hidden="true"
    viewBox="0 0 24 24"
    className={className}
...
export function ProjectLinks({ links, variant = "detail", className }: ProjectLinksProps) {
  if (!links) return null;

  const visibleKeys = variant === "card" ? new Set<keyof ProjectLinkMap>(["github"]) : null;
      ...
      {entries.map(({ key, label, Icon }) => {
        ...
        return (
          <a
            ...
            title={label}
          >
            <Icon className={variant === "detail" ? "h-3.5 w-3.5" : "h-4 w-4"} />
            {variant === "detail" ? <span>{label}</span> : <span className="sr-only">{label}</span>}
          </a>
        );
      })}
    </div>
